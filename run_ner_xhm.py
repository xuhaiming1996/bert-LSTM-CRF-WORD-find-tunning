...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    features["segment_ids"] = tf.train.Feature(
        bytes_list=tf.train.BytesList(value=[numpy.array(feature.segment_ids).astype(numpy.int64).tostring()])
    )
...
def file_based_input_fn_builder(input_file, max_word_length,max_sen_length, is_training,
  ...
  name_to_features = {
      ...
      "segment_ids": tf.FixedLenFeature([], tf.string),
...
def create_model(bert_config,
  ...
  return (loss, trans_params, logits, sequence_lengths,bacth_size, max_sen_len, max_word_len,input_label_ids,input_label_mask)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):
    ...
    (loss,trans_params,logits,sequence_lengths,bacth_size, max_sen_len, max_word_len,input_label_ids,input_label_mask) = create_model(
        bert_config,
        is_training,
        input_words_ids,
        input_pos_ids,
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(input_label_ids,
          ...
          predictions, _ = tf.contrib.crf.crf_decode(logits, trans_params, sequence_lengths)

          conf_mat = metrics.get_metrics_ops(input_label_ids, predictions, num_labels,input_label_mask)
      ...
      eval_metrics = (metric_fn,[input_label_ids,input_label_mask,logits,bert_config.ntags,trans_params,sequence_lengths])
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=loss,
      ...
      def crf_decodr(logits, trans_params,sequence_lengths):
          predictions,_=tf.contrib.crf.crf_decode(logits,trans_params,sequence_lengths)
          return predictions,sequence_lengths

      predictions, sequence_lengths = crf_decodr(logits,trans_params,sequence_lengths)
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    with tf.gfile.GFile(output_eval_file, "w") as writer:
      ...
      pre, rec, f1 = metrics.get_metrics(result["eval_cm"], bert_config.ntags)
      tf.logging.info("eval_precision: {}".format(pre))
      writer.write("eval_precision: {}".format(pre))
      tf.logging.info("eval_recall: {}".format(rec))
      writer.write("eval_recall: {}".format(rec))
      tf.logging.info("eval_f1: {}".format(f1))
      writer.write("eval_f1: {}".format(f1))
      tf.logging.info("  %s = %s", "eval_accuracy", str(result["eval_accuracy"]))
      writer.write("%s = %s\n" % ("eval_accuracy", str(result["eval_accuracy"])))
